...
biggie <- dplyr::left_join(ann, a2g, by = "peptide_id")
colnames(biggie)[1] <- "target_id"

so <- sleuth_prep(samples,  num_cores = 6, target_mapping = biggie)
...
lrt_results$gene_name <- tolower(lrt_results$gene_name)
colorgenes <- dplyr::left_join(colors, lrt_results, by = "gene_name")

## Order them by 'significance'
colorgenes <-  colorgenes[order(colorgenes$qval),]

# Drop color genes that don't have an associated transcript
colorgenes <- dplyr::filter(colorgenes, target_id != "NA")

## These are just the 'statistically significant' hits
sig_colorgenes <- dplyr::filter(colorgenes, qval < 0.05)



##### This will iterate through and make a nice figure for each of the statistically significant color genes
for (i in 1:nrow(sig_colorgenes)){
  transcript <- sig_colorgenes[i,"target_id"]
  gene <- sig_colorgenes[i,"gene_name"]
  qval <- sig_colorgenes[i,"qval"]
...
write.csv(sig_colorgenes, "imitator_significant_color_gene_stats.csv")

dim(sig_colorgenes)
significant_colorgenes <- unique(sig_colorgenes$gene_name)
```

XXXX DE transcripts accounting for batch effects.
...
sopop <- sleuth_prep(samples, num_cores = 6, target_mapping = biggie)
...
colorgenes_pop <- dplyr::left_join(colors, pop_lrt_results, by = "gene_name")

## Order them by 'significance'
colorgenes_pop <-  colorgenes_pop[order(colorgenes_pop$qval),]

# Drop color genes that don't have an associated transcript
colorgenes_pop <- dplyr::filter(colorgenes_pop, target_id != "NA")

## These are just the 'statistically significant' hits
sig_colorgenes_pop <- dplyr::filter(colorgenes_pop, qval < 0.05)



##### This will iterate through and make a nice figure for each of the statistically significant color genes
for (i in 1:nrow(sig_colorgenes_pop)){
  transcript <- sig_colorgenes_pop[i,"target_id"]
  gene <- sig_colorgenes_pop[i,"gene_name"]
  qval <- sig_colorgenes_pop[i,"qval"]
...
write.csv(sig_colorgenes_pop, "imitator_population_significant_color_gene_stats.csv")

significant_population_colorgenes <- unique(sig_colorgenes_pop$gene_name)
```



Looks like there are ### significant transcripts (q val < 0.05) between the populations here.
...
table(nopop_lrt_results[,"qval"] < 0.05) ### significant transcripts between the null and the others...
...
table(noweek_lrt_results[,"qval"] < 0.05) 
...
table(nointer_lrt_results[,"qval"] < 0.05) 
...
so.int2 <- sleuth_fit(so.int2, formula = full_design, fit_name = "full2")
so.int2 <- sleuth_fit(so.int2, formula = nopop_design, fit_name = "nopop2")
...
table(nopop2_lrt_results[,"qval"] < 0.05) ### significant transcripts between the null and the others...
...
table(noweek2_lrt_results[,"qval"] < 0.05)
