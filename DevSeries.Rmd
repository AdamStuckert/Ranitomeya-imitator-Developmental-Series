...
#Background
This uses the pseudo-quantifications from Kallisto to undergo differential expression analyses. These pseudo-quantification results should be in a folder entitled "kallisto_quants" within the R project folder
...
# Sleuth sets #cores to 1 in Rstudio
# Can change the available cores using:
# options(mc.cores = 4L)

setwd("C:/Users/Adam Stuckert/Documents/R/R projects/Developmental Series")
...
Plot the PCA to see how things fall out, and also make it very pretty. 


```{r}
plot_pca(so, point_size = 0.001) + geom_point() + aes(size = 3, colour = samples$locality, shape = factor(samples$week)) + scale_colour_manual(values = c("yellow1","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) + scale_shape_manual(values=c(15,16,17,18), guide = guide_legend(title = "Timepoint (weeks)", override.aes = list(size=4))) + guides(size=FALSE)
...
This is not quite as clear as the PCA was from EdgeR and a previously made transcriptome from adult skin. Oh well. Saved that for my presentation!


That was just with the estimated counts, what about with tpm?
...
```{r}
plot_pca(so, units = "tpm", point_size = 0.001, text_labels = TRUE) #+ geom_point() + aes(size = 3, colour = samples$locality, shape = factor(samples$week)) + scale_colour_manual(values = c("yellow2","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) + scale_shape_manual(values=c(15,16,17,18), guide = guide_legend(title = "Timepoint (weeks)", override.aes = list(size=4))) + guides(size=FALSE)

#ggsave("pca-subsamp-pop-tpm.pdf")
#ggsave("pca-subsamp-pop-tpm.tiff", width = 6.81, height = 3.99)
```

Alright, so S2-8 is the clear outlier here. At the end of all of this, I might reanalyze this data without that particular sample. FOr now, what is driving this
...
```{r}
# This is the model...
# so <- sleuth_prep(samples, full_model = spline_design)
...
So there are 6,962 differentially expressed transcripts...which is a lot. I guess I should do Wald test, because this package doesn't support likelihood ratio tests.

```{r}
#plot_ma(so_wt, "ns(week, df = 3)3", test_type = 'wt')
#plot_volcano(so_wt, "ns(week, df = 3)3", test_type = 'wt')
#plot_transcript_heatmap(so, head(lrt_results, n = 20)$target_id, 'est_counts')
```


Get a kallisto abundance table...which I won't print since it is ~2 million rows but I can send either of you if you are interested.

```{r}
kal_table <- kallisto_table(so)
```
...
Looks like there are 9,701 significant transcripts (q val < 0.05) between the populations here.

Now I will make the same PCA I did above with the prepped data for the timeseries to see if there is a difference between the two. I don't think there should be though.

```{r}
plot_pca(sopop, units = "tpm", point_size = 0.001) + geom_point() + aes(size = 3, colour = samples$locality, shape = factor(samples$week)) + scale_colour_manual(values = c("yellow2","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) + scale_shape_manual(values=c(15,16,17,18), guide = guide_legend(title = "Timepoint (weeks)", override.aes = list(size=4))) + guides(size=FALSE)

#ggsave("pca-subsamp-pop.pdf")
#ggsave("pca-subsamp-pop.tiff", width = 6.81, height = 3.99)
```

Nope, this is the same. I'll double check to see if the principal components loadings are the same (I believe they should be).

```{r}
plot_loadings(sopop, pc_input = 1)
plot_loadings(sopop, pc_input = 2)
```

Ok, good to know that I was right about that. 
...
a <- ggplot(tmp, aes(x=week, y=est_counts)) + geom_point(aes(size = 3, color = locality)) + scale_colour_manual(values = c("yellow2","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) +  geom_smooth(method = loess) + ggtitle(paste0(gene, "\n(", transcript, "), q value = ", qval)) + guides(size=FALSE)
 ...
 print(a)
  ...
  ggsave(paste0("pcloadings-figures/", gene, "-", transcript, "-spline.tiff"), width = 6.81, height = 3.99)
}
...
a <- ggplot(tmp, aes(x=week, y=est_counts)) + geom_point(aes(size = 3, color = locality)) + scale_colour_manual(values = c("yellow2","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) +  geom_smooth(method = loess) + ggtitle(paste0(gene, "\n(", transcript, "), q value = ", qval)) + guides(size=FALSE)
 ...
 print(a)
  ...
  ggsave(paste0("pcloadings-figures/", gene, "-", transcript, "-pop.tiff"), width = 6.81, height = 3.99)
}
...
Plotting specific color genes from the Xenopus protein database. This bit will print the figures to the Markdown output (si it is long). First with q values from the spline analysis:
...
```{r}
#color <- data.frame(transcripts = character(27), genes = character(27))
#color$transcripts <- c("Transcript_23342","Transcript_68280", "Transcript_41319", "Transcript_23342", "Transcript_68280", "Transcript_23342", "Transcript_68280", "Transcript_20866", "Transcript_20866", "Transcript_12016", "Transcript_19946", "Transcript_68867", "Transcript_31054", "Transcript_28405", "Transcript_8404", "Transcript_64029", "Transcript_64031", "Transcript_8404", "Transcript_49748", "Transcript_21442", "Transcript_75275", "Transcript_62041", "Transcript_32702", "Transcript_11861", "Transcript_35108", "Transcript_25521", "Transcript_63155", "Transcript_23342")
...
a <- ggplot(tmp, aes(x=week, y=est_counts)) + geom_point(aes(size = 3, color = locality)) + scale_colour_manual(values = c("yellow","orange1", "red1"), guide = guide_legend(title = "Population", override.aes = list(size=4))) +  geom_smooth(method = loess) + ggtitle(paste0(gene, "\n(", transcript, "), q value = ", qval)) + guides(size=FALSE)
 ...
 print(a)
  ...
  ggsave(paste0("colorgene-figures/", gene, "-", transcript, "-pop.tiff"), width = 6.81, height = 3.99)
}
```
...
# Now merge the model results with the annotation data...
tmp <- merge(lrt_results, targets, by = "target_id", all = TRUE, sort = FALSE)
    ## I believe this worked. Went through a few of them to verify...

# Now to see if I can do some GO analyses...
  # Use topGO in R. 
  # install it
...
```{r}
############Attempting to do the biomart bit in parallel because it took forever.

# This should theoretically work to do it in parallel
...
require(dplyr)
foreach(i= 1:nrow(xenann)) %dopar% {
  id <- xenann[i,1]
  t <- xenann[i,2]
    # Now get the stats values for the transcript
  tmp = 
  # tmp.targets =
  tmp <- dplyr::filter(t3g, Protein.stable.ID.version == t)
  tmp <- tmp[,-1]
  if (nrow(tmp) == 0) {tmp <- data.frame(Protein.stable.ID.version = NA, Transcript.stable.ID = NA, Version..transcript. = NA, Gene.stable.ID = NA, Gene.name = NA, Gene.description = NA, Transcript_type = NA, Gene.stable.ID.1 = NA, GO.term.accession = NA, GO.term.name = NA, GO.term.definition = NA, GO.term.evidence.code = NA, GO.domain = NA, GOSlim.GOA.Accession.s. = NA, GOSlim.GOA.Description = NA)}
  tmp <- cbind(id, t, tmp)
  targets <- rbind(targets, tmp)
...
stopCluster(cl)
### Holy shit, this took so FUCKING long to run. I need to parallelize this fucker.
